...
func TestMarketingNotificationStrategy_CheckLimitAndSend(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.callUpdate {
				mockRepo.On("UpdateUserLimit", mock.Anything, "Marketing", mock.AnythingOfType("repository.Limit")).Return()
			}

			strategy := strategies.NewMarketingNotificationStrategy(2, 30*24*time.Hour, mockRepo, mockTimeProvider)
